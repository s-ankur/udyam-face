...
class imcrop:
    ...
    def crop(self):
        ...
        while True:
            ...
            if self.mode > 0:
                cv2.rectangle(
                    img2, self.bounding_box[0], self.current_pos, (0, 255, 0), 1)
        ...
        if len(self.bounding_box) != 2 or self.bounding_box[0][0] == self.bounding_box[
                1][0] or self.bounding_box[0][1] == self.bounding_box[1][1]:
...
def imdilate(img, kernel='circle', size=5, iterations=1):
    return cv2.dilate(
        img.copy(), _kernel(
            kernel, (size, size)), iterations=iterations)
...
def imopen(img, kernel='circle', size=5):
    return cv2.morphologyEx(
        img.copy(), cv2.MORPH_OPEN, _kernel(
            kernel, (size, size)))


def imclose(img, kernel='circle', size=5):
    return cv2.morphologyEx(
        img.copy(), cv2.MORPH_CLOSE, _kernel(
            kernel, (size, size)))
...
def im2bw(img, otsu=True, threshold=127):
    img = im2gray(img)
    (thresh, im_bw) = cv2.threshold(img, threshold, 255,
                                    cv2.THRESH_BINARY | (cv2.THRESH_OTSU * int(otsu)))
...
def blend_transparent(face_img, overlay_img):
    ...
    face_img[:, :, :] = np.uint8(cv2.addWeighted(
